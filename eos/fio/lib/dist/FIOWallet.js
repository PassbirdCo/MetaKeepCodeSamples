...
class FIOWallet {
  ...
  constructor(options) {
    ...
    if (
      ![
        utils_1.Environment.PRODUCTION,
        utils_1.Environment.DEVELOPMENT,
      ].includes(this.options.env)
    ) {
      throw new Error(
   ...
   */
  async mapHandle(fioHandle, publicAddresses, reason) {
    ...
    const fioPubKey = (0, utils_1.EOSPubKeyToFIOPubKey)(
      wallet.wallet.eosAddress
    );
    const actionData = {
    ...
    const { rawTx, serializedActionData, chainId } = await (0,
    utils_1.createRawTx)({
      publicKey: fioPubKey,
