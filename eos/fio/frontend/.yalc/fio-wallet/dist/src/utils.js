...
const FIO_ABI_MAP = new Map([
    [
        "fio.address",
        {
            ...
            abi_hash: "19a099047a11ee6c2a1d86b44b9100eba44ef85b60e33edfbdfcc58ff6ab7c56",
            account_name: "fio.address",
            code_hash: "72c258fbcb0b328960bdbc4fc521d76cc53e5517bb5f5f2bf991d08030ddae6b",
        },
    ],
]);
/**
 ...
 */
const createRawTx = async ({ publicKey, actionData, account, action, fioBaseUrl, }) => {
    ...
    Transactions_1.Transactions.abiMap.set(fioAddressAbi.account_name, fioAddressAbi);
...
const getChainData = async (fioBaseUrl) => {
    try {
        // Fetch chain info
        const chainInfoResponse = await axios_1.default.post(`${fioBaseUrl}/chain/get_info`);
        const chainInfo = chainInfoResponse.data;
        // Fetch block data using the last irreversible block number
        const blockDataResponse = await axios_1.default.post(`${fioBaseUrl}/chain/get_block`, {
 ...
 */
const broadcastTx = async ({ rawTx, chainId, account, signature, fioBaseUrl, }) => {
    const { fioAddressAbi } = await getABI(account);
    Transactions_1.Transactions.abiMap.set(fioAddressAbi.account_name, fioAddressAbi);
    const transaction = new Transactions_1.Transactions();
    ...
    try {
        ...
        const pushTransactionResponse = await axios_1.default.post(`${fioBaseUrl}/chain/push_transaction`, pushTransactionArgs);
        const pushTransactionJson = pushTransactionResponse.data;
