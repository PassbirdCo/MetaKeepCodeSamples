...
describe("CustomERC721 contract", function () {
  ...
  before(async function () {
    ...
    customERC721WithProxy = await customERC721.attach(
      customERC721Proxy.address
    );

    await customERC721WithProxy.initialize(
      "MetaKeep Originals",
      "MTKP",
      owner.address
    );
  ...
  describe("Whitelist", function () {
    it("Should add an address to the whitelist", async function () {
      ...
      expect(await customERC721WithProxy.isWhitelisted(addr1.address)).to.equal(
        true
      );
    });
  ...
  describe("Minting", function () {
    it("Should not mint if the address is not whitelisted", async function () {
      await expect(
        customERC721WithProxy.mint(addr1.address, 123)
