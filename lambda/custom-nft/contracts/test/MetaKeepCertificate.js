...
describe('Token', function () {
  ...
  before(async function () {
    MetaKeepCertificate = await ethers.getContractFactory('MetaKeepCertificates');
  ...
  describe('Mint And Destroy Certificate', function () {
    ...
    it('Should not be able to transfer certificate as it is soulbound tokens', async function () {
      await expect(certificate.connect(addr1).transferFrom(addr1.address,addr2.address, 100)).to.be.revertedWith('Cannot transfer');
    });
